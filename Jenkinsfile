...
//            slackSend(color: "good", message: "Success .. Please check the logs")
            build wait: false, propagate: false, job: 'pipeline_sample_succ', parameters: [string(name: 'JenkinsURL', value: '${JenkinsURL}'), string(name: 'JOBNAME', value: '${JOB_NAME}'), string(name: 'BUILDID', value: '${BUILD_ID}')], waitForStart: true
...
//            slackUploadFile(channel: "#notification", filePath: "log_${BUILD_TIMESTAMP}.txt")
        ...
        failure { 
            build wait: false, propagate: false, job: 'pipeline_sample_fail', parameters: [string(name: 'JenkinsURL', value: '${JenkinsURL}'), string(name: 'JOBNAME', value: '${JOB_NAME}'), string(name: 'BUILDID', value: '${BUILD_ID}')], waitForStart: true
//            build job: 'pipeline_sample_fail', parameters: [string(name: 'JenkinsURL', value: '${JenkinsURL}'), string(name: 'JOBNAME', value: '${JOB_NAME}'), string(name: 'BUILDID', value: '${BUILD_ID}')]
